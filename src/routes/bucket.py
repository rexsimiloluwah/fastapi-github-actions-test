...
def create_bucket(bucket_data:BucketSchema,db:Session=Depends(get_db),user_id:int=Depends(auth_helper.auth_wrapper)):
    return bucket_controller.create_new_bucket(db,bucket_data,user_id)

@router.post("/buckets",response_model=dict)
def create_buckets(buckets_data:List[BucketSchema],db:Session=Depends(get_db),user_id:int=Depends(auth_helper.auth_wrapper)):
    return bucket_controller.create_new_buckets(db,buckets_data,user_id)

@router.put("/bucket/{bucket_id}",response_model=dict)
def update_bucket(bucket_id:int,bucket_data:BucketUpdateSchema,db:Session=Depends(get_db),user_id:int=Depends(auth_helper.auth_wrapper)):
...
def delete_bucket(bucket_id:int,db:Session=Depends(get_db),user_id:int=Depends(auth_helper.auth_wrapper)):
