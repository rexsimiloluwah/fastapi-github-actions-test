...
def create_access_token(db:Session,email:str,password:str):
    ...
    if not user:
        raise HTTPException(status_code=404, detail={"status":False,"message":"User does not exist."})
    if not user.check_password(password):
        raise HTTPException(status_code=400, detail={"status":False,"message":"Password is incorrect."})
